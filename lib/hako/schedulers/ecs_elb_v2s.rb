...
module Hako
  module Schedulers
    class EcsElbV2s
      # @param [String] app_id
      # @param [String] region
      # @param [Hash] elb_v2s_config
      # @param [Boolean] dry_run
      def initialize(app_id, region, elb_v2s_config, dry_run:)
        @app_id = app_id
        @region = region
        @elb_v2s_config = elb_v2s_config
        @dry_run = dry_run
      end
      ...
      def modify_attributes
        ...
        @elb_v2s_config.each do |elb_config|
          unless load_balancer_given?(elb_config)
            load_balancer = describe_load_balancer(elb_config)
            set_subnets(elb_config, load_balancer)
            set_listeners(elb_config, load_balancer)
            set_load_balancer_attributes(elb_config, load_balancer)
          end
      ...
      # @return [Boolean]
      def load_balancer_given?(elb_config)
        elb_config.key?('load_balancer_name')
      end

      # @return [String]
      def elb_name(elb_config)
        elb_config.fetch('load_balancer_name', "hako-#{@app_id}")
      end

      # @return [Aws::ElasticLoadBalancingV2::Types::LoadBalancer]
      def describe_load_balancer(elb_config)
        elb_client.describe_load_balancers(names: [elb_name(elb_config)]).load_balancers[0]
      rescue Aws::ElasticLoadBalancingV2::Errors::LoadBalancerNotFound
        nil
      end

      def describe_tg(elb_config)
        elb_client.describe_target_groups(names: [target_group_name(elb_config)]).target_groups[0]
      rescue Aws::ElasticLoadBalancingV2::Errors::TargetGroupNotFound
        nil
      end

      # @return [String]
      def target_group_name(elb_config)
        elb_config.fetch('target_group_name', "hako-#{@app_id}")
      end

      # @return [Boolean]
      def target_group_given?(elb_config)
        elb_config.key?('target_group_name')
      end
      ...
      # @return [nil]
      def create_target_group(elb_config)
        elb_type = elb_config.fetch('type', nil)
      ...
      # @return [nil]
      def set_listeners(elb_config, load_balancer)
        new_listeners = elb_config.fetch('listeners')
        if load_balancer
